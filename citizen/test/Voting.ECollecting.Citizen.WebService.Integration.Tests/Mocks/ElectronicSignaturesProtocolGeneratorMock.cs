...
public class ElectronicSignaturesProtocolGeneratorMock(IDmDocDataSerializer dmDocDataSerializer, DmDocConfig config)
    : PdfGeneratorMock<ECollectingProtocolTemplateData, ECollectingProtocolDataContainer>(dmDocDataSerializer), IElectronicSignaturesProtocolGenerator
{
    ...
    protected override string BuildFileName(ECollectingProtocolTemplateData data)
        => $"{data.Collection.Description}_{config.ElectronicSignaturesProtocolFileName}";
}
