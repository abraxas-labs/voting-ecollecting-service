...
public class InitiativeGetElectronicSignaturesProtocolTest : BaseRestTest
{
    public InitiativeGetElectronicSignaturesProtocolTest(TestApplicationFactory factory)
        : base(factory)
    {
    }
    ...
    public override async Task InitializeAsync()
    {
        ...
        await MockedDataSeeder.Seed(
            ...
            SeederArgs.Initiatives.WithInitiatives(
                ...
                InitiativesMuStGallen.GuidSignatureSheetsSubmitted));
    }
    ...
    {
        await AssertStatus(
            async () => await ReaderClient.GetAsync(BuildUrl(InitiativesMuStGallen.IdSignatureSheetsSubmitted)),
            HttpStatusCode.NotFound);
    }
    ...
    public async Task WorksInStates(CollectionState state)
    {
        await RunOnDb(db => db.Initiatives
            .Where(x => x.Id == InitiativesCtStGallen.GuidUnitySignatureSheetsSubmitted)
    ...
    private static string BuildUrl(string collectionId = InitiativesCtStGallen.IdUnitySignatureSheetsSubmitted)
