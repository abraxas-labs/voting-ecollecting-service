...
public class CollectionListMunicipalitySignatureSheetsTest : BaseGrpcTest<CollectionMunicipalityService.CollectionMunicipalityServiceClient>
{
    public CollectionListMunicipalitySignatureSheetsTest(TestApplicationFactory factory)
        : base(factory)
    {
    }
    ...
    {
        var response = await CtSgStichprobenverwalterClient.ListSignatureSheetsAsync(NewValidRequest());
    ...
    public async Task ShouldWorkAsMu()
    {
        ...
        var response = await MuSgStichprobenverwalterClient.ListSignatureSheetsAsync(req);
        await Verify(response);
    }

    [Fact]
    public async Task ShouldBeEmptyWithUnknownBfs()
    {
        var req = NewValidRequest(x => x.Bfs = "9999");
        var response = await CtSgStichprobenverwalterClient.ListSignatureSheetsAsync(req);
        response.SignatureSheets.Should().BeEmpty();
    }
    ...
    {
        await AssertStatus(
            async () => await MuSgStichprobenverwalterClient.ListSignatureSheetsAsync(NewValidRequest()),
            StatusCode.NotFound);
    }
    ...
    public async Task ShouldThrowAsCtOnMu()
    {
        ...
        await AssertStatus(
            async () => await CtSgStichprobenverwalterClient.ListSignatureSheetsAsync(req),
            StatusCode.NotFound);
    }
    ...
    public async Task ShouldThrowOtherMuTenant()
    {
        ...
        await AssertStatus(
            async () => await MuGoldachKontrollzeichenerfasserClient.ListSignatureSheetsAsync(req),
    ...
    public async Task WorksInState(CollectionState state)
    {
        ...
        if (state.IsEnded())
        {
            await CtSgStichprobenverwalterClient.ListSignatureSheetsAsync(NewValidRequest());
        }
        else
        {
            await AssertStatus(
                async () => await CtSgStichprobenverwalterClient.ListSignatureSheetsAsync(NewValidRequest()),
                StatusCode.NotFound);
        }
    }
    ...
    protected override async Task AuthorizationTestCall(GrpcChannel channel)
    {
        await new CollectionMunicipalityService.CollectionMunicipalityServiceClient(channel)
            .ListSignatureSheetsAsync(NewValidRequest());
    }
    ...
    {
        var req = new ListCollectionMunicipalitySignatureSheetsRequest
        {
