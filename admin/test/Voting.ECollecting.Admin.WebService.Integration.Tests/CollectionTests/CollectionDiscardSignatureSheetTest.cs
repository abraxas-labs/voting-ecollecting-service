...
public class CollectionDiscardSignatureSheetTest : BaseGrpcTest<CollectionSignatureSheetService.CollectionSignatureSheetServiceClient>
{
    ...
    public async Task ShouldWork()
    {
        ...
        sheet.State.Should().Be(CollectionSignatureSheetState.NotSubmitted);
    }
    ...
    public async Task TestAuditTrail()
    {
        ...
        {
            await CtSgStichprobenverwalterClient.DiscardAsync(NewValidRequest());
    ...
    public async Task ShouldWorkAsMu()
    {
        ...
        var response = await MuSgStichprobenverwalterClient.DiscardAsync(req);
        await Verify(response);
    ...
    public async Task ShouldThrowAsCtOnMu()
    {
        ...
        req.SignatureSheetId = _sheetMuSgId.ToString();
        await AssertStatus(
            async () => await CtSgStichprobenverwalterClient.DiscardAsync(req),
            StatusCode.NotFound);
    }
    ...
    {
        await AssertStatus(
            async () => await MuSgStichprobenverwalterClient.DiscardAsync(NewValidRequest()),
            StatusCode.NotFound);
    }
    ...
    public async Task ShouldThrowOtherTenant()
    {
        ...
        req.SignatureSheetId = CollectionSignatureSheets.BuildGuid(
            ...
            4).ToString();
        await AssertStatus(
            async () => await MuSgStichprobenverwalterClient.DiscardAsync(req),
            StatusCode.NotFound);
    }
    ...
    public async Task WorksInState(CollectionState state)
    {
        ...
        {
            await AssertStatus(
                async () => await CtSgStichprobenverwalterClient.DiscardAsync(NewValidRequest()),
                StatusCode.NotFound);
        }
        else
        {
    ...
    private static DiscardSignatureSheetRequest NewValidRequest()
    {
        return new DiscardSignatureSheetRequest
        {
